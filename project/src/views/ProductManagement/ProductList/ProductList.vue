...
export default {
  ...
  methods: {
    ...
    search() {
        ...
        this.axios.get(`http://127.0.0.1:474/product/search?cateName=${cateName}&keyWord=${keyWord}`)
            .then(response => {
    ...
    getproductList() {
      // 发送ajax请求 获取所有数据
      this.axios.get("http://127.0.0.1:474/product/productlist").then(response => {
    ...
    getproductListByPage() {
      ...
      // 发送ajax请求 按照分页请求数据
      this.axios
        .get(`http://127.0.0.1:474/product/productlistbypage?currentPage=${currentPage}&pageSize=${pageSize}`)
        .then(response => {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          // 收集修改后的新数据 和 一个原来的id
            ...
            this.axios.post('http://127.0.0.1:474/product/saveedit',
            qs.stringify(params),
  ...
  // 生命周期钩子函数 只要进入组件 组件就会经历这个周期 会自动触发这个函数
  created() {
