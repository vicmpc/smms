...
export default {
  ...
  methods: {
    ...
    handleEdit(cardid) {
      ...
      this.axios.get(`http://127.0.0.1:888/users/editmember?cardid=${cardid}`)
        .then(response => {
          // 一一对应 把数据回填到模态框里面
    ...
    batchDel () {
      ...
      this.axios.post('http://127.0.0.1:888/users/batchdel', 
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.axios.post('http://127.0.0.1:888/users/saveedit',
          qs.stringify(params),
